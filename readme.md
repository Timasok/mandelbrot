...
4. Оно конвертируется в цвет, который приобретает точка с координатами (X0, Y0). Снизу приведён код функции ``Color getColor(int n)``.
``
...
```C++
    ...
    return getColor(n);                             // Конвертация n в некоторый пикселя - (4) 
```
...
//////////////////
Опишем также порядок обхода пикселей.
...
!!! самое главной сравнивать с таким же для О3
