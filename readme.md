...
Число `n` - количество итераций до выхода из цикла. Внутри цикла реккурсивно вычисляются координаты числа ```Z(n+1)``` по формулам:
...
Мы не позволяем компилятору отбросить вычисления даже если они нигде не используются. Для создаём ```volatile``` переменную ```n_res```. После каждого вычисления присваиваем ей значение ```n```. Таким образом алгоритм сохранит свою асимптотику, независящую от аппаратуры.
...
```
Большинство ``_mm_`` функций тут используют векторный тип данных ```__m128```. Он представляет из себя 4 float-а по 32-бита. За одну итерацию цикла вычисляется в 4 раза больше значений ```n``` и интуитино кажется, что алгоритм должен ускорится как минимум в 4х.
