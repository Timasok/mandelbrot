...
Большинство ``_mm_`` функций, представленных тут, используют векторный тип данных ``__m128``. Он представляет из себя 4 float-а по 32-бита. За одну итерацию цикла вычисляется в 4 раза больше значений ``n`` и интуитивно кажется, что алгоритм должен ускорится как минимум в 4х.
...
```
Порядок обхода после приминения SSE-оптимизации(обход по столбцам идёт с шагом 4).
