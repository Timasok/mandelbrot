...
```
Большинство ``_mm_`` функций тут используют векторный тип данных ```__m128```. Он представляет из себя 4 float-а по 32-бита. За одну итерацию цикла вычисляются в 4 раза больше значений ```n``` и интуитино кажется, что алгоритм должен ускорится как минимум в 4х.
