...
Мно́жество Мандельбро́та — множество точек на комплексной плоскости, для которых рекуррентное соотношение ``Z(n+1)^2 = Z(n)^2+Z0`` при ``Z0 = 0`` задаёт ограниченную последовательность.(Википедия)

Данный фрактал обычно отображается на комплексной или декартовой плоскости. Для каждой точки в пределах видимости экрана вычисляется число `n`. Далее оно конвертируется в цвет, который и приобретает точка.

Число `n` - количество итераций до выхода из цикла. Внутри цикла рекурсивно вычисляются координаты числа ```Z(n+1)``` по формулам:

``X(n+1) = X(n)^2 - Y(n)^2 + X0``

``Y(n+1) = 2*X(n)*Y(n) + Y0``

Условие выхода из цикла следующее:
...
```
Ключевое слово ```volatile``` возле переменной придает ей статус неоптимизируемой. Таким образом все вычисления, в которых она участвует, компилятор оставляет без оптимизации. 

Мы не позволяем компилятору отбросить вычисления даже если они нигде не используются. Для создаём ```volatile``` переменную ```n_res```. После каждого вычисления присваиваем ей значение ```n```. Таким образом алгоритм сохранит свою асимптотику, не зависящую от аппаратуры.
...
```
Большинство ``_mm_`` функций тут используют векторный тип данных ```__m128```. Он представляет из себя 4 float-а по 32-бита. За одну итерацию цикла вычисляется в 4 раза больше значений ```n``` и интуитивно кажется, что алгоритм должен ускорится как минимум в 4х.
...
Назовём **1-м режимом - вычисления без отображением**, а *2-м режимом - вычисления с отображением*.
...
Основная задача проекта была выполнена. Мы научились снижать систематическую погрешность измерения времени работы алгоритма и оптимизировать его с помощью SIMD-инструкций. Нашему процессору доступны интринсики типа ```AVX2``` и ```AVX-512```, не нашедшие отражения в данной работе, из-за чего не было достигнуто максимально возможное быстродействие. 

Однако результат использования ```SSE4``` тоже велик: в 1-м и 2-м экспериментах удалось ускорить алгоритм в 7-8 раз. Отсюда следует, что параллелизм значительно повышает производительность. 
