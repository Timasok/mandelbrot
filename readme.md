...
$R(n)=\sqrt{X(n)^2 + Y(n)^2} > R_{max} = 10$ или $n > N_{max} = 256$.
...
В результате использования ```SSE4``` удалось ускорить алгоритм в **3.6** раза. Это подтверждает гипотезу о том, что параллелизм повышает производительность. 
...
В результате использования ```SSE4``` удалось ускорить алгоритм в **2.6** раза. Это подтверждает гипотезу о том, что параллелизм повышает производительность. 
